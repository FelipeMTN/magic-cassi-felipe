...
export type TableProps<T = any> = React.ComponentProps<typeof Column> & {
  data: T[];
...
export const Table: React.FC<TableProps> = ({
  ...
  return (
    <Column fillWidth radius="l" border="neutral-alpha-weak" {...flex}>
      <Row fillWidth horizontal="between" paddingY="12" paddingRight="12" paddingLeft="24" gap="16" vertical="center" xs={{direction: "column-reverse", horizontal: "start"}}>
        <Row fillWidth gap="12" vertical="center" horizontal="between">
          {selectable && someCurrentSelected ? (
            ...
            typeof heading === "string" ? (
              <Text variant="label-default-m" paddingY="12">{heading}</Text>
            ) : (
              <Row paddingY="12">{heading}</Row>
            )
          )}
        </Row>
        {showSearch && (
          <Row maxWidth={16} data-border="rounded">
            <Input
              height="s"
              id="search"
              placeholder="Search..."
